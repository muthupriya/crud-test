...
conn.connect(function(err) {
  if (err){
    ...
    */
    const siteTitle = 'System test for crud';
    ...
    */
    app.get('/', function(req, res){
      conn.query("SELECT * FROM events ORDER BY e_id DESC", function(err, result){
    ...
    app.get('/edit/:id', function(req, res){
      ...
      conn.query('SELECT * FROM events WHERE e_id = "'+req.params.id+'"', function(err, result){
        ...
        res.render('pages/edit-event', {
          siteTitle: siteTitle,
          pageTitle: 'Edit : '+result[0].e_name,
          item: result[0]
        });
      });
    });
    ...
    app.post('/edit', function(req, res){
      ...
      */
      ...
      query += ' e_start_date = "'+dateFormat(now, "yyyy-mm-dd")+'", ';
      query += ' e_end_date = "'+dateFormat(now, "yyyy-mm-dd")+'", ';
